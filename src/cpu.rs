...
type AddrModeF<M> = fn(&mut P65, &mut M,  fn(&mut P65));
...
impl P65 {
    ...
    fn fetch_op<M: Bus>(&mut self, mem: &mut M) -> u8 {
    ...
    fn a1_ac<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
    ...
    fn a1_imp<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { mem.read(self.pc as usize); },      // discard read
    ...
    fn a2_ix<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
    ...
    fn a2_imm<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
    ...
    fn a2_zp<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a2_abs<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a2_iy<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.v1 =  mem.read(self.pc as usize);  self.inc_pc(); },
    ...
    fn a2_zpx<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a2_zpy<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a2_ay<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize);  self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize);  self.inc_pc(); 
    ...
    fn a2_ax<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize);  self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize);  self.inc_pc();
    ...
    fn a3_zp<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a3_abs<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a3_ix<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize);  self.inc_pc(); },
    ...
    fn a3_ax<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize);  self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize);  self.inc_pc();
                                self.v2 =  ((self.al as u32 + self.x as u32) >> 8) as u8;
    ...
    fn a3_ay<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize);  self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize);  self.inc_pc();
                                self.v2 =  ((self.al as u32 + self.y as u32) >> 8) as u8;
    ...
    fn a3_zpx<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a3_zpy<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn a3_iy<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.v1 =  mem.read(self.pc as usize);  self.inc_pc(); },
            2 => { self.al =  mem.read(self.v1 as usize);  self.v1 = self.v1.wrapping_add(1); },
            3 => { self.ah =  mem.read(self.v1 as usize);  
    ...
    fn a4_zp<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
            2 => { self.v1 =  mem.read(self.al as usize);                },
    ...
    fn a4_zpx<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
            2 => { mem.read(self.al as usize); self.al = self.al.wrapping_add(self.x); },                 //discard read
            3 => { self.v1 =  mem.read(self.al as usize); },
    ...
    fn a4_ax<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize);  self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize);  self.inc_pc();
                                self.v2 =  ((self.al as u32 + self.x as u32) >> 8) as u8;
    ...
    fn a4_abs<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc();   },
            2 => { self.ah =  mem.read(self.pc as usize); self.inc_pc();   },
            3 => { self.v1 =  mem.read(self.ah_al() as usize);             },
    ...
    fn jsr_abs<M: Bus>(&mut self, mem: &mut M, _: fn(&mut Self)) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn brk_imp<M: Bus>(&mut self, mem: &mut M, _: fn(&mut Self)) {
        match self.ts {
    ...
    fn rti_imp<M: Bus>(&mut self, mem: &mut M, _: fn(&mut Self)) {
        match self.ts {
    ...
    fn jmp_abs<M: Bus>(&mut self, mem: &mut M, _: fn(&mut Self)) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn jmp_ind<M: Bus>(&mut self, mem: &mut M, _: fn(&mut Self)) {
        match self.ts {
            1 => { self.al =  mem.read(self.pc as usize); self.inc_pc(); },
            2 => { self.ah =  mem.read(self.pc as usize); self.inc_pc(); },
    ...
    fn rts_imp<M: Bus>(&mut self, mem: &mut M, _: fn(&mut Self)) {
        match self.ts {
            1 => { mem.read(self.pc as usize);                            self.inc_pc(); },   // discard read
            2 => { mem.read((self.s as usize) + 0x100);                   self.inc_sp(); },     // discard read too
    ...
    fn a5_bxx<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
    ...
    fn a5_plx<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
            1 => { mem.read(self.pc as usize); },                                 // discard read
    ...
    fn a5_phx<M: Bus>(&mut self, mem: &mut M, opfun: OpcodeF) {
        match self.ts {
    ...
    fn ad_unk<M: Bus>(&mut self, _: &mut M, _: fn(&mut Self)) {
    ...
    fn decode_addr_mode<M: Bus>(op: u8) -> AddrModeF<M> {
        match op {
    ...
    pub fn reset<M: Bus>(&mut self, mem: &mut M) {
        self.s =   0xFD;
        self.op =  0x00;
    ...
    pub fn jump<M: Bus>(&mut self, mem: &mut M, address: u16) {
        self.pc =  address;
        self.fetch_op(mem);
        self.tick();
    }
    ...
    pub fn run<M: Bus>(&mut self, mem: &mut M, count: u64) -> u64 {
     ...
     */
    pub fn step<M: Bus>(&mut self, mem: &mut M, count: u64) {
        let mut count = count;
        while count > 0 {
            self.run(mem,1);
...
pub fn status_string<M: Bus>(pr: &P65, mem: &mut M) -> String {
    use disasm;
    let op = pr.op;
